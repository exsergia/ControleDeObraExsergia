  ...
  const handleExport = () => {
    ...
    const currentData = activeTab === 'materiais' 
      ...
      : filteredAtividades.map(a => ({
          ...
          'Valor Orçado': (a.quantidadePrevista * (a.valorUnitario || 0)).toFixed(2),
          'Valor Executado': (a.quantidadeExecutada * (a.valorUnitario || 0)).toFixed(2)
        }));
    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 px-6 py-3 bg-zinc-900 text-white rounded-xl font-bold hover:bg-zinc-800 transition-all shadow-xl shadow-zinc-200 active:scale-95"
        >
...
<<<<<<< HEAD
           ...
           <div className="relative z-10 space-y-2 mt-2">
              <div className="flex items-center justify-between">
                 <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-tight flex items-center gap-1.5">
                    <Package className="w-3 h-3" /> Materiais
                 </span>
                 <span className="text-sm font-black">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(stats.matTotal)}
                 </span>
              </div>
              <div className="flex items-center justify-between">
                 <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-tight flex items-center gap-1.5">
                    <Activity className="w-3 h-3" /> Progresso (Mão de Obra)
                 </span>
                 <span className="text-sm font-black">

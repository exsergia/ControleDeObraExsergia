...
function ScannerModal({ onSuccess, onClose }: { onSuccess: (text: string) => void, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-zinc-900/60 backdrop-blur-sm">
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.95 }}
        className="bg-white w-full max-w-2xl rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl max-h-[92vh] sm:max-h-[90vh] flex flex-col"
      >
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right shrink-0 min-w-[120px] space-y-2">
            ...
            <div>
              ...
              <div className="text-xs font-bold text-zinc-900">
                {inDate ? formatMovementDate(inDate) : '--/--/----'}
              </div>

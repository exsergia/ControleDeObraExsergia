...
export default function Materiais() {
  ...
  const filtered = materiais.filter(m => {
    ...
    const matchObra = filtroObraId === 'all' || m.obraId === filtroObraId;
    return matchBusca && matchObra;
  });

  // Resumo de gastos e quantidade de materiais por obra (para visualizar
  // para onde foi cada material e quanto foi gasto em cada obra).
  const resumoPorObra = obras
    .map(obra => {
      const matsDaObra = materiais.filter(m => m.obraId === obra.id);
      const totalGasto = matsDaObra.reduce((acc, m) => acc + (Number(m.valorTotal) || 0), 0);
      const totalItens = matsDaObra.length;
      const totalQuantidade = matsDaObra.reduce((acc, m) => acc + (Number(m.quantidade) || 0), 0);
      return { obra, totalGasto, totalItens, totalQuantidade };
    })
    .filter(r => r.totalItens > 0)
    ...
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative sm:w-64">
          <select
            className="w-full pl-4 pr-10 py-2 bg-white border border-zinc-200 rounded-lg text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-zinc-900/10 shadow-sm"
            value={filtroObraId}
      ...
      {/* Resumo: para onde foi cada material e gasto por obra */}
      {resumoPorObra.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Gastos de materiais por obra</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {resumoPorObra.map(({ obra, totalGasto, totalItens, totalQuantidade }) => (
              <button
                key={obra.id}
                onClick={() => setFiltroObraId(filtroObraId === obra.id ? 'all' : obra.id)}
                className={cn(
                  'text-left p-4 rounded-xl border shadow-sm transition-all',
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <Package className={cn('w-4 h-4', filtroObraId === obra.id ? 'text-white' : 'text-zinc-400')} />
                  <span className="font-bold text-sm truncate">{obra.nome}</span>
                </div>
                <p className={cn('text-lg font-black', filtroObraId === obra.id ? 'text-white' : 'text-zinc-900')}>
                  {fmtBRL(totalGasto)}
                </p>
                <p className={cn('text-[10px] font-bold uppercase tracking-tight mt-1', filtroObraId === obra.id ? 'text-zinc-300' : 'text-zinc-400')}>
                  {totalItens} {totalItens === 1 ? 'entrega' : 'entregas'} • {totalQuantidade} itens
                </p>
              </button>
            ))}
          </div>

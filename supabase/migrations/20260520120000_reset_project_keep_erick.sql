-- RESET CONTROLADO DO PROJETO EXSERGIA
-- Destrutivo: apaga dados do app e storage.
-- Mantem somente o usuario Auth com email nascimentoerick446@gmail.com.

create extension if not exists "pgcrypto";

-- 1) Mantem apenas o usuario principal no Supabase Auth.
delete from auth.users
where lower(coalesce(email, '')) <> 'nascimentoerick446@gmail.com';

-- 2) Garante tabelas do app.
...
-- Os buckets sao recriados/garantidos abaixo; para apagar arquivos antigos,
-- use a Storage API ou esvazie os buckets pelo Dashboard.
...
create index if not exists idx_tool_logs_status on public."toolLogs" ((data ->> 'statusLog'));
create index if not exists idx_tool_logs_data_saida on public."toolLogs" ((data ->> 'dataSaida') desc);
create index if not exists idx_tool_logs_tool_status_saida on public."toolLogs" ((data ->> 'toolId'), (data ->> 'statusLog'), (data ->> 'dataSaida'));
create index if not exists idx_tool_logs_return_photo_path on public."toolLogs" ((data ->> 'fotoDevolucaoPath'))
  where coalesce(data ->> 'fotoDevolucaoPath', '') <> '';
create index if not exists idx_materiais_obra_status on public.materiais ((data ->> 'obraId'), (data ->> 'statusConferencia'));
create index if not exists idx_materiais_data_entrega on public.materiais ((data ->> 'dataEntrega') desc);
create index if not exists idx_atividades_obra on public.atividades ((data ->> 'obraId'));

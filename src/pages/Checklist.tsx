...
export default function ChecklistPage() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      sendBrowserNotification('Checklist Enviado!', `Relatório da obra ${obras.find(o => o.id === selectedObraId)?.nome} finalizado.`);
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        notify('warning', 'Arquivo Muito Grande', 'A foto para evidência deve ter no máximo 5MB.');
        return;
      }
      setPhotoFile(file);
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      {activeStep === 4 && (
        <section className="space-y-6 animate-in slide-in-from-right duration-300">
          ...
          <div className="space-y-4">
            <label className="text-sm font-bold text-zinc-600 uppercase tracking-widest pl-1">Foto de Evidência / Ocorrência</label>
            <div className="relative group/photo">
              {photoPreview ? (
                ...
                <label className="flex flex-col items-center justify-center w-full aspect-video bg-white border-2 border-dashed border-zinc-200 rounded-2xl cursor-pointer hover:border-zinc-400 hover:bg-zinc-50 transition-all group-hover/photo:scale-[1.005]">
                  <Camera className="w-12 h-12 text-zinc-400 mb-2 group-hover:text-zinc-600 transition-colors" />
                  <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Tirar Foto do Campo</p>

  ...
  const handleAddObra = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'obras'), {
        ...
        operadoresIds: formData.equipe?.map(e => e.operatorId) || [],
            ...
            <form onSubmit={handleAddObra} className="p-6 space-y-5">
              ...
              <div className="space-y-3 pt-2 text-left">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-1">Equipe e Níveis</label>
                  <span className="text-[10px] text-zinc-400 font-medium">Selecione e atribua um nível</span>
                </div>
                
                <div className="max-h-48 overflow-y-auto border border-zinc-100 rounded-xl p-2 space-y-2 bg-zinc-50/50">
                  {operadores.map(op => {
                    const equipeEntry = formData.equipe?.find(e => e.operatorId === op.id);
                    const isSelected = !!equipeEntry;
                      ...
                      <div
                        ...
                        onClick={() => {
                          setFormData({
                            ...formData,
                            equipe: isSelected
                              ? formData.equipe?.filter(entry => entry.operatorId !== op.id)
                              : [...(formData.equipe || []), { operatorId: op.id, nivel: op.funcao || 'Operador' }]
                          });
                        }}
                        onKeyDown={(e) => {
                        ...
                        className={cn(
                          "flex items-center gap-3 p-2 rounded-lg transition-all cursor-pointer select-none",
                          isSelected ? "bg-white shadow-sm border border-zinc-100" : "opacity-60 hover:opacity-100"
                        )}>
                        <input 
                          type="checkbox"
                          checked={isSelected}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setFormData({
                                ...formData,
                                equipe: [...(formData.equipe || []), { operatorId: op.id, nivel: op.funcao || 'Operador' }]
                              });
                            } else {
                              setFormData({
                                ...formData,
                                equipe: formData.equipe?.filter(entry => entry.operatorId !== op.id)
                              });
                            }
                          }}
                          className="w-4 h-4 rounded border-zinc-300 focus:ring-zinc-900 text-zinc-900 pointer-events-none"
                        />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-zinc-900 truncate">{op.nome} {op.sobrenome}</p>
                        </div>
                        {isSelected && (
                          <select
                            className="w-28 px-2 py-1 bg-zinc-100 border-none rounded text-[10px] font-bold uppercase focus:ring-1 focus:ring-zinc-900/10 cursor-pointer"
                            value={equipeEntry.nivel}
                            onClick={(e) => e.stopPropagation()}
                            onChange={(e) => {
                              setFormData({
                                ...formData,
                                equipe: formData.equipe?.map(entry =>
                                  entry.operatorId === op.id ? { ...entry, nivel: e.target.value } : entry
                                )
                              });
                            }}
                          >
                            ...
                            {op.funcao && !['Operador','Encarregado','Supervisor','Técnico','Engenheiro'].includes(op.funcao) && (
                              <option value={op.funcao}>{op.funcao}</option>
                            )}
                          </select>
                        )}
                      </div>
                  ...
                  {operadores.length === 0 && (
                    <p className="text-center py-4 text-xs text-zinc-400">Nenhum operador cadastrado.</p>
                  )}
                </div>
              </div>
